...
import com.timepoorprogrammer.saml.impls.standard.consumer.processor.SAML11ResponseValidatorImpl;
import com.timepoorprogrammer.saml.common.FactoryBase;
import com.timepoorprogrammer.saml.impls.standard.consumer.processor.SAML11ResponseValidatorImpl;
...
/**
 * SAML1.1 Response validator factory.  As a SAML consumer, we may need to apply a bespoke validator for SAML1.1
 ...
 * and is called SAML11ResponseValidatorImpl. If a bespoke processor isn't found then the factory
 * provides the default SAML1.1 implementation.
 * <p/>
 ...
 * <li>Implement the SAML11ResponseValidator interface</li>
 * <li>Hold the class name SAML11ResponseValidatorImpl</li>
 * </ul>
 * <p/>
 ...
 * @author Jim Ball
 */
public class SAML11ResponseValidatorFactory extends FactoryBase {
    private static final Logger log = LoggerFactory.getLogger(SAML11ResponseValidatorFactory.class);

    /**
     * Bespoke SAML11ResponseValidators have to live in the consumer.processor subpackage of a
     * bespoke implementation.
     */
    private static final String BESPOKE_IMPL_SUB_PACKAGE_LOCATION = "consumer.processor";

    /**
     * As a SAML consumer, we may need to apply a custom response validator for SAML 1.1 content sent by a remote
     * identity provider.
     *
     * @param customerCode Customer code of identity provider within Northgate
     * @return instance of a SAML11ResponseValidator
     */
    @SuppressWarnings("unchecked")
    public static SAML11ResponseValidator getInstance(final String customerCode) {
        if (customerCode != null) {
            final List<String> details = parseClassName(SAML11ResponseValidator.class.getName());
            ...
            try {
                ...
                return (SAML11ResponseValidator) implementation.newInstance();
                ...
                return new SAML11ResponseValidatorImpl();
